...
const ministers: MinisterAccountSeed[] = [
  ...
  {
    name: "Francisca Toledo Echegaray",
...
const ministerAccounts: AccountConfig[] = ministers.map((minister) => ({
  id: slug(minister.name),
  ...minister,
  accountType: "minister",
  aliases: [
    minister.name,
    minister.name.split(" ").at(-1) ?? minister.name,
