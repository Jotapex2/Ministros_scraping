...
const ollamaCachePath = path.join(process.cwd(), ".sessions", "ollama-models.json");
...
export async function POST(request: Request) {
  ...
  if (!allowRequest(requestKey(request, "llm"), 40))
  ...
  try {
    ...
    const provider = body.llmProvider || process.env.LLM_PROVIDER || "ollama";
    const host = resolveOllamaHost(body.ollamaHost);
    const model = body.ollamaModel || process.env.OLLAMA_MODEL || "gemma3:1b";
