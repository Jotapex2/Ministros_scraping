...
const topicAssignmentSchema = z.object({
  id: z.string(),
  topicName: z.string().min(1).max(80),
...
function topicJsonSchema(ids: string[]) {
  return {
    type: "object",
    properties: {
      assignments: {
        ...
        items: {
          ...
          properties: {
            id: { type: "string", enum: ids },
            topicName: { type: "string", minLength: 1, maxLength: 80 },
          ...
          required: ["id", "topicName", "summary", "keywords"],
          additionalProperties: false,
        },
      },
    },
...
function repairText(value: string) {
  let repaired = value;
  for (let attempt = 0; attempt < 2 && /(?:Ã|Â|â|ï¿)/.test(repaired); attempt++) {
    ...
    sentiment === "positive"
      ...
      : sentiment === "negative"
        ...
        : sentiment === "neutral"
          ...
          : rawScore;
  const target =
    typeof value.target === "string" ? repairText(value.target.trim()) : "";
  ...
  return resultSchema.parse({
    ...
    reasonShort:
      typeof value.reasonShort === "string"
        ? repairText(value.reasonShort)
        : "ClasificaciÃ³n del modelo local sin explicaciÃ³n.",
    topic:
      typeof value.topic === "string" && value.topic.trim()
        ? repairText(value.topic.trim())
        : "Sin clasificar",
    keywords: stringArray(value.keywords).map(repairText),
    entities: stringArray(value.entities).map(repairText),
  });
}
    ...
    const response = await fetchWithRetry(
      `${cleanHost}/api/generate`,
      {
        ...
        body: JSON.stringify({
          model: modelName,
          system,
          prompt: `REGLAS OBLIGATORIAS PARA CADA ENTRADA:
1. Devuelve exactamente un resultado por id y conserva los ids sin cambios.
2. Analiza el sentimiento hacia el objeto evaluado, no el tono general ni la popularidad.
3. negative solo si hay critica, rechazo, acusacion o desacuerdo explicito hacia un objeto; positive solo si hay apoyo, elogio o aprobacion explicita.
4. neutral para informacion factual, anuncios, citas sin postura propia o textos sin evaluacion; uncertain solo ante ironia, ambiguedad o contexto insuficiente.
5. score va de -1 a 1: negative es menor que 0, positive mayor que 0, neutral y uncertain son 0.
6. confidence va de 0 a 1. No inventes hechos, cifras, nombres, entidades ni intenciones fuera del texto.
7. targetKind debe ser una sola categoria valida. reasonShort, topic, keywords y entities deben ser breves y basarse solo en el texto.
8. Responde solo el JSON solicitado, sin Markdown ni comentarios.

Analiza estas ${batch.length} entradas y devuelve ${batch.length} resultados en el mismo orden:
${JSON.stringify(compactItems)}`,
  ...
  try {
    const batchSize = /(^|[:_-])1b($|[:_-])/i.test(modelName) ? 4 : 12;
...
export async function labelTopicsOllama(
  ...
  const compactItems = items.map((item) => ({
    id: item.id,
    text: item.text.slice(0, 900),
    sentiment: item.sentiment || "uncertain",
  }));
  const prompt = `Eres un clasificador de temas de conversación política chilena.
Tu tarea es asignar exactamente un tema a cada pieza. El tema describe el acontecimiento, decisión, política pública, conflicto o asunto concreto del texto.

REGLAS OBLIGATORIAS:
1. Devuelve exactamente ${items.length} asignaciones, una por cada id recibido, y conserva cada id sin cambios.
2. No inventes hechos, causas, cifras, personas, instituciones, intenciones ni contexto que no aparezca en el texto.
3. No uses temas genéricos como "política", "gobierno", "actualidad", "redes sociales" o "ministros" si el texto permite identificar un asunto más concreto.
4. Reutiliza un nombre del catálogo existente solamente cuando describe el mismo acontecimiento o asunto. Si no corresponde, crea un nombre breve y específico.
5. Si el texto no entrega evidencia suficiente para identificar un asunto, usa topicName "Sin clasificar", summary "No hay evidencia suficiente para identificar un tema" y keywords [].
6. topicName debe ser breve, en español, y tener como máximo 80 caracteres. summary debe ser factual y tener como máximo 240 caracteres.
7. keywords debe contener entre 0 y 5 palabras o frases breves que estén explícitas o sean una formulación directa del texto.
8. La salida debe ser exclusivamente JSON válido, sin Markdown, explicaciones ni texto antes o después.

Catálogo existente (solo referencias permitidas): ${JSON.stringify(existing)}

Piezas:
${JSON.stringify(compactItems)}

Devuelve este esquema exacto: {"assignments":[{"id":"id recibido","topicName":"tema","summary":"resumen factual","keywords":["palabra"]}]}`;
  ...
  try {
    const response = await fetchWithRetry(
      `${cleanHost}/api/generate`,
      {
        ...
        body: JSON.stringify({
          model: modelName,
          prompt,
          format: topicJsonSchema(items.map((item) => item.id)),
          stream: false,
          think: false,
          keep_alive: "10m",
          options: {
            temperature: 0,
            num_ctx: 4096,
            num_predict: Math.min(2500, 180 + items.length * 130),
          },
        }),
        cache: "no-store",
      },
    ...
    const parsed = parseJsonResponse<{ assignments?: unknown[] }>(content);
    ...
    for (const assignment of parsed.assignments) {
      if (assignment && typeof assignment === "object") {
        const id = String((assignment as Record<string, unknown>).id ?? "");
        if (items.some((item) => item.id === id)) byId.set(id, assignment);
      }
    }
    const assignments = items.map((item) => {
      ...
      const topicName = typeof value.topicName === "string" && value.topicName.trim()
        ? repairText(value.topicName.trim()).slice(0, 80)
        : "Sin clasificar";
      const summary = typeof value.summary === "string"
        ? repairText(value.summary.trim()).slice(0, 240)
        : "No hay evidencia suficiente para identificar un tema";
      const keywords = topicName === "Sin clasificar"
        ? []
        : stringArray(value.keywords)
        .map((keyword) => repairText(keyword.trim()).slice(0, 40))
        .filter(Boolean)
        .slice(0, 5);
      return topicAssignmentSchema.parse({
        id: item.id,
        topicName,
        summary,
        keywords,
    ...
    return topicResponseSchema.parse({ assignments });
  } catch (error) {
    throw new ProviderError(
